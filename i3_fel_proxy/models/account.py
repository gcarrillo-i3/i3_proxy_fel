...
class AccountMove(models.Model):
    ...
    def construir_correlativo_interno(self):
        ...
        if correlativo:
            ...
            if es_corr_interno == True:
                if  diario == 1:                                  
                    corr = self.env['ir.sequence'].next_by_code('account.facturas')
            else:            
                corr = False
    ...
    def manejar_errores(self, mensaje, res_json, longitud_datos):
        bot_odoo = self.env.ref('base.partner_root').id        
        if int(longitud_datos) > 0:                     
            ...
            if error_txt:
                ...
                error = error                
            self.message_post(body=f'No se publicó la factura por error del certificador FEL: {mensaje} {error}', author_id=bot_odoo)
        else:
            self.message_post(
                body=f'No se publicó la factura por error del certificador FEL: {mensaje}', author_id=bot_odoo)    
    ...
    def solicitud_dte(self):                
        if self.journal_id.generar_fel == True:
            for factura in self:                
                ...
                direccion_receptor = factura.partner_id.street or "Guatemala"
                ...
                for lineas in factura.invoice_line_ids:
                    ...
                    precio_por_linea = lineas.price_unit * lineas.quantity                                                                                                
                    cantidad_precio = (precio_por_linea)                    
                    
                    # revisamos si hay descuento
                    if lineas.discount >= 1:
                        logging.warning("En descuento")
                        porcentaje_descuento = lineas.discount / 100                        
                        # limitamos decimales en descuento
                        porcentaje_descuento = round(porcentaje_descuento, 4)                        
                        # obtenemos el valor del  descuento del precio de linea
                        precio_total_descuento =  precio_por_linea * descuento
                        # deducimos el descuento en precio linea
                        precio_por_linea = precio_por_linea - precio_total_descuento
                    ...
                    if tipo_dte in ["FACT", "FCAM", "NCRE"]:
                            raw_impuesto_por_linea = ( precio_por_linea / 1.12) * 0.12                            
                            #impuesto con decimales
                            impuesto_por_linea = round(raw_impuesto_por_linea,4);                            
                            #caculo de impuesto por - precio linea
                            monto_gravable_linea = precio_por_linea - impuesto_por_linea                            
                            #acumlador de total monto gravale
                            total_monto_gravable_lineas = total_monto_gravable_lineas + monto_gravable_linea                              
                            total_monto_linea = monto_gravable_linea + impuesto_por_linea
                        
                    elif tipo_dte == 'FESP':
                        if tax_ids_list_5 == tax_ids_odoo:                         
                             #calculo del iva por linea
                            impuesto_por_linea = ( precio_por_linea / 1.12) * 0.12
                            impuesto_por_linea = round(raw_impuesto_por_linea,4);                            
                            #calclulo de base sin impuesto
                            monto_gravable_linea = precio_por_linea/ 1.12                            
                            #calculo del isrtotal_retencion_lineas
                            fix_retencion_linea = monto_gravable_linea * round(isr_proveedor_05,4)                            
                            total_retencion_lineas =  + fix_retencion_linea                            
                            #calcluo de total de la linea
                            total_monto_gravable_lineas = total_monto_gravable_lineas+ monto_gravable_linea                                                                                    
                            #agregamos el resultado del fesp
                            total_menos_retenciones = total_monto_gravable_lineas - total_retencion_lineas                                                                                    
                            total_impuestos_lineas = total_impuestos_lineas + impuesto_por_linea
                            # logging.warning(total_menos_retenciones)
                    # Excentos       
                    elif tipo_dte == 'FEXP':
                        direccion_consignatario = f'{pais_receptor}, {municipio_receptor}, {departamento_receptor}'
                        monto_gravable_linea  = precio_por_linea                            
                        total_monto_linea = monto_gravable_linea + impuesto_por_linea
                    ...
                    detalle_lineas = {
                        ...
                        "monto_impuesto":impuesto_por_linea,
                        "total":total_monto_linea                        
                    }                
                    
                    detalles.append(detalle_lineas)                    
                    total_monto_factura +=  monto_gravable_linea
                    total_impuestos_factura += + impuesto_por_linea
                    total_factura = total_impuestos_factura + total_monto_factura
                #exit()
                ...
                if  dict_factura['tipo_dte'] in ["FACT", "FESP","FCAM","NCRE","FEXP"]: 
                    ...
                    if int(status_code) == 200:                                              
                        if int(exito) == 1:
                            ...
                            factura.message_post(body=f'REPUESTA DEL CERTIFICADOR: {mensaje}',author_id=bot_odoo)
